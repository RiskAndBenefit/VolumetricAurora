...
#if WITH_EDITOR
	//DFBakerComponent = CreateDefaultSubobject<USplineDFTextureBakerComponent>(TEXT("SplineDFTextureBakerComponent"));
	//if (DFBakerComponent && RootComponent)
	//{
	//	DFBakerComponent->SetupAttachment(RootComponent);
	//}
#endif
...
#if WITH_EDITOR
	if (!DFBakerComponent && GIsEditor && !IsTemplate())
	{
		UClass* DFBakerClass = StaticLoadClass(UObject::StaticClass(), nullptr, TEXT("/Script/VolumetricAuroraEditor.SplineDFTextureBakerComponent"));

		if (DFBakerClass)
		{
			DFBakerComponent = NewObject<UDFBakerComponentBase>(this, DFBakerClass);
			if (DFBakerComponent && RootComponent)
			{
				DFBakerComponent->CreationMethod = EComponentCreationMethod::Instance;
				DFBakerComponent->SetFlags(RF_Transactional);

				this->AddInstanceComponent(DFBakerComponent);

				DFBakerComponent->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepRelativeTransform);
				DFBakerComponent->OnComponentCreated();
				DFBakerComponent->RegisterComponent();
...
void AVolumetricAurora::SimulateAuroraPass(UPotentialFlowAuroraPreset* FlowPreset, float DeltaTime)
{
	...
	if (bControlPointsDirty)
	{
		...
		for (const FAuroraFlowElement& CP : FlowPreset->ControlPoints)
		{
	...
	(FRHICommandListImmediate& RHICmdList)
	{
		...
		if (bResetSimulation)
		{
			// Restore from checkpoint if available, otherwise clear to black
			if (FlowPreset->HasValidCheckpoint())
			{
				// Get checkpoint Texture2D resource
				FTextureResource* CheckpointResource = FlowPreset->SimulationCheckpointTexture->GetResource();

				if (CheckpointResource && CheckpointResource->TextureRHI)
				{
					// Validate resolution matches current RenderTarget
					int32 CheckpointWidth = FlowPreset->SimulationCheckpointTexture->GetSizeX();
					int32 CheckpointHeight = FlowPreset->SimulationCheckpointTexture->GetSizeY();
					int32 CurrentWidth = FrontBufferTexture->Desc.Extent.X;
					int32 CurrentHeight = FrontBufferTexture->Desc.Extent.Y;

					if (CheckpointWidth == CurrentWidth && CheckpointHeight == CurrentHeight)
					{
						// Register checkpoint Texture2D as RDG texture
						FRDGTextureRef CheckpointTexture = GraphBuilder.RegisterExternalTexture(
							CreateRenderTarget(CheckpointResource->TextureRHI, TEXT("CheckpointTexture"))
						);

						// Copy checkpoint to both FrontBuffer and BackBuffer
						// This ensures complete restoration of ping-pong buffer state
						AddCopyTexturePass(GraphBuilder, CheckpointTexture, BackBufferTexture);
					}
					else
					{
						// Resolution mismatch: cannot restore, clear instead
						AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(BackBufferTexture), FLinearColor::Black);
					}
				}
				else
				{
					// Fallback: checkpoint texture resource not ready
					AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(BackBufferTexture), FLinearColor::Black);
				}
			}
			else
			{
				// No checkpoint: initialize with empty state
				AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(BackBufferTexture), FLinearColor::Black);
			}
		}
		...
		{
			FControlPointInfoGPU DummyData = {};
			GraphBuilder.QueueBufferUpload(ControlPointInfoBuffer, &DummyData, sizeof(FControlPointInfoGPU));
		}
		...
		{
			FSingleForceControlPointGPU DummyData = {};
			GraphBuilder.QueueBufferUpload(SingleForceCPBuffer, &DummyData, sizeof(FSingleForceControlPointGPU));
		}
		...
		{
			FDoubleForceControlPointGPU DummyData = {};
			GraphBuilder.QueueBufferUpload(DoubleForceCPBuffer, &DummyData, sizeof(FDoubleForceControlPointGPU));
		}
		...
		{
			FTripleForceControlPointGPU DummyData = {};
			GraphBuilder.QueueBufferUpload(TripleForceCPBuffer, &DummyData, sizeof(FTripleForceControlPointGPU));
		}
		...
		{
			FDipoleControlPointGPU DummyData = {};
			GraphBuilder.QueueBufferUpload(DipoleCPBuffer, &DummyData, sizeof(FDipoleControlPointGPU));
		}
		...
		{
			FCurlControlPointGPU DummyData = {};
			GraphBuilder.QueueBufferUpload(CurlCPBuffer, &DummyData, sizeof(FCurlControlPointGPU));
		}
		...
		{
			FWarpControlPointGPU DummyData = {};
			GraphBuilder.QueueBufferUpload(WarpCPBuffer, &DummyData, sizeof(FWarpControlPointGPU));
		}
...
void AVolumetricAurora::RenderControlPointsDebug() const
{
	...
	for (const FAuroraFlowElement& ControlPoint : PFAuroraPreset->ControlPoints)
	{
		...
		case EControlPointType::Emitter:
			...
			AttenuationStart = ControlPoint.EmissionAttenuationStart;
			AttenuationEnd = ControlPoint.EmissionAttenuationEnd;
			break;
		case EControlPointType::Attenuator:
			DisplayColor = FColor::Turquoise;
			AttenuationStart = ControlPoint.FadeAttenuationStart;
			AttenuationEnd = ControlPoint.FadeAttenuationEnd;
			break;
		default:
...
AVolumetricAurora* AVolumetricAurora::CreatePreviewAurora()
{
	...
	// Hide all DFBoundVisualizerComponents (wireframe plane visualizers)
	...
	{
		if (Comp->GetClass()->GetName().Contains(TEXT("DFBoundVisualizer")))
		{
...
void AVolumetricAurora::CreatePreviewSceneCapture()
{
	...
	// Manual capture mode for real-time preview
	// We call CaptureScene() explicitly in UpdatePreviewAurora() to control timing
	// bCaptureEveryFrame = false prevents "major inefficiency" warning from redundant captures
	PreviewSceneCapture->bCaptureEveryFrame = false;
	PreviewSceneCapture->bCaptureOnMovement = false;
	PreviewSceneCapture->bAlwaysPersistRenderingState = true;
...
void AVolumetricAurora::DestroyPreviewAurora()
{
	...
	// Step 2: Clear Preview Actor reference (don't call Destroy() directly)
	// Actor will be cleaned up when World is destroyed
	// Calling Destroy() on EditorPreview world actors causes "World has no context" warning
	...
	if (PreviewWorld)
	{
		...
		// DestroyWorld properly cleans up all actors without context warnings
		// CleanupWorld + DestroyWorld ensures proper cleanup sequence
